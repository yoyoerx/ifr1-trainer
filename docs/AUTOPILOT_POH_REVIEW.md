...
|---|---|---|---|
| R1 | Sec.3.1.2 p.3-4: at full-scale CDI deflection the AP establishes a **45 deg** intercept to the selected course; less than 45 only when closure rate is high. | GPS legs: 45 deg cut (`nav_intercept_deg`, F49). **VOR/LOC (VLOC) path: proportional, 22 deg per unit deflection capped at 30 deg** (`_VLOC_GAIN`, `_MAX_INTERCEPT`) - at full scale that is a 22 deg cut. | GPS **OK**, VLOC **GAP** |
| R2 | p.3-4: turn onto the course "will always begin between 100% and 20% CDI needle deflection", earlier at faster closure rate. | GPS legs: turn-in distance = lead of a 90%-rate turn x1.5, clamped to 20-100% of full scale (constant in closure rate). VLOC: none - a linear ramp. | GPS **PARTIAL**, VLOC **GAP** |
| R3 | p.3-4: **capture at 15%** deflection: gain steps down. | GPS annunciator clears at 15% of the CDI scale. No gain step. VLOC clears at 75% deflection (`_CAPTURE_DEFLECTION`). | **PARTIAL** |
| R4 | p.3-4: staged authority after capture - **CAP** (90% std-rate limit), **CAP SOFT** at +15 s (45% std rate), crosswind correction established at +30 s, **SOFT** at +75 s (15% std rate); "at 115 kts, 2.4 nm is travelled in 75 s". | Single law throughout; no timed stages, wind correction is instant (`_drift`, F44). | **N/M** |
| R5 | p.3-5: in SOFT the AP "ignores short term CDI needle deflections" (no scalloping over station passage); >50% for 60 s reverts to CAP SOFT. | Not modelled. | **N/M** |
| R6 | p.3-5: **NAV flashes** when deflection >50% or flag in view (FAIL also on flag). | No flashing/FAIL. | **N/M** |
| R7 | p.3-4: engaging with course pointer within 5 deg of the course and <10% deflection goes **straight to SOFT**. | Not modelled (always the same law). | **N/M** |
| R8 | p.3-5: pressing **APR while tracking in NAV** (within 50%) raises authority to CAP SOFT; a **new course >=10 deg** different reverts to CAP. | `press_apr` swaps lateral to APR; no course-change reversion. | **PARTIAL** |
...
| R10 | Sec.4.1 p.4-3: turn-rate limit **90% of standard rate** (HDG, NAV, APR, REV, CWS; piston), **GPSS 90/110/130%** by hardware mod code. | `SimModel` turns at a fixed 3.0 deg/s (100%) for every mode (`_STD_RATE_DEG_S`). | **GAP** |
| R11 | Sec.3.1.2 p.3-4: **NAV mode uses the course selected on the HSI/OBS** and the CDI needle; the pilot must keep it set. GPSS (sec.3.1.3) is the mode that follows the GPS's own course, and ignores the course pointer. | NAV with the GNS as CDI source flies the GPS **DTK automatically** (`_gps_track_command`) - i.e. it behaves as GPSS; NAV vs GPSS differ only by a flag. The 530's "Set course to ###" (p.80) reminder is not modelled. | **GAP** |
...
| R16 | Sec.1.4 / 2.1 p.2-3: AP master turns the unit on and the display reads **RDY** - **no roll mode is engaged** until a mode button is pressed; ALT/VS/CWS "can only be engaged" with a roll mode engaged (sec.3.1.4, 3.1.5, 4.2). | Master press engages an invented **LVL** roll base ("wings level"); ALT/VS press engages the AP by themselves (`press_alt` -> `engage()`). | **GAP** |
...
| # | POH says | Trainer does | Status |
|---|---|---|---|
| A1 | Sec.3.3.3, 3.3.4: LOC/VOR **front course** inbound: **APR** button = NAV APR, intercepts and tracks. | `press_apr`, VLOC path with the 22 deg law (see R1). | **PARTIAL** |
...
| A3 | Sec.3.2.1.1 p.3-12: **GS arms automatically** after 1 s (rev 5+; 10 s rev 4-) when: NAV APR + **ALT engaged** + no flags + LOC freq + within **50%** of LOC + **>10%** GDI below the glideslope. | GS armed by pressing APR, no conditions (`armed_vert = GS`). | **GAP** |
| A4 | p.3-12: GS **captures at 5% GDI** below centerline; ALT annunciation drops. | `_maybe_capture_gs` captures at **75%** of full scale (0.5 deg) - and only then does vertical speed follow the beam. | **GAP** |
| A5 | p.3-12: pressing **APR** with GS armed disables it (GS flashes); again re-arms (1 s / 10 s). Pressing **ALT** above the beam engages GS manually; caution >20% above - "aggressive". | APR press toggles the whole APR mode. No GS disable/re-arm, no manual-engage rule. | **GAP** |
| A6 | p.3-13: GS annunciation **flashes** at >50% GDI or flag (with FAIL). | Not modelled. | **N/M** |
