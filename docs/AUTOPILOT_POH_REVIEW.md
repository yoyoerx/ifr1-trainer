...
`STEC_55X_Cirrus_POH_2ndEd.pdf` is the earlier (2nd Ed., Nov 2007) System 55X POH for the Avidyne-PFD Cirrus
(SHA-256 `33e62887aba55b011770618f788f318cf49be5a2df5af11d0d4d875876a75c2c`, from `stpeteair.org/wp-content/uploads/55XCirrusAvidyne.pdf`): the same intercept sequence in
different words ("HIGH / INTERMEDIATE / LOW GAIN" for CAP / CAP SOFT / SOFT), the "gradually shallow the intercept
angle" sentence, and "The GPSS mode should be used anytime the selected navigation source is the GPS".
...
| R2 | p.3-4: turn onto the course "will always begin between 100% and 20% CDI needle deflection", earlier at faster closure rate. | F57: the POH bounds (turn begins between 100% and 20% of full scale, earlier at higher closure rate, closure rate measured in needle units so station distance counts) are enforced; inside them the trainer shallows the cut gradually - the Cirrus 2nd Ed. POH says "gradually shallow the intercept angle as the autopilot turns the aircraft onto the course" - along a curve the POH does not publish (11.5 s of closure + the 15% band). | **OK** (bounds from the POH, curve is the trainer's) |
...
| R6 | p.3-5: **NAV flashes** when deflection >50% or flag in view (FAIL also on flag). | F53/F57: `Autopilot.flashing` (NAV/APR/REV beyond 50% or a flag; NAV+GPSS with no course) and the steady **FAIL** annunciation (`Autopilot.fail`) with a NAV flag or no GPSS course. | **OK** |
...
| R8 | p.3-5: pressing **APR while tracking in NAV** (within 50%) raises authority to CAP SOFT; a **new course >=10 deg** different reverts to CAP. | F57: APR pressed while tracking within 50% keeps the captured course and drops SOFT to CAP SOFT; a new course >= 10 deg reverts to CAP (F50). | **OK** |
| R9 | Sec.3.1.2.1 / 3.1.3.1 p.3-6: **pilot-selectable intercept angle** - set the heading bug to the intercept heading, hold HDG, press NAV (twice for GPSS); AP flies that heading until it must turn to avoid overshoot. | Not implemented - decided against (2026-09-21): the trainer flies the POH default 45 deg cut. | **SKIPPED** (by decision) |
...
| A8 | Sec.3.5.1 p.3-34: with a WAAS GPS, GPSS flies the lateral approach; **NAV APR on the FAC** captures the GPS glideslope (LPV, LNAV/VNAV, LNAV+V). | F55/F56: on a `--unit 530w` the GPS LPV / L/VNAV glidepath, and the LNAV+V / LP+V advisory glidepath, drive NAV APR while the CDI is on GPS (same arm / 5% capture logic as an ILS). | **OK** |
...
| # | POH says | Trainer does | Status |
|---|---|---|---|
| O1 | Sec.3.1.6: **CWS** (control-wheel steering) holds turn rate and VS. | Not implemented - decided against (2026-09-21). | **SKIPPED** (by decision) |
...
| O3 | Sec.2.1/2.2: power-up test annunciations, FAIL states. | Not implemented: the trainer always starts airborne, so there is no start-up sequence. Back burner. | **DEFERRED** |
...
| O5 | Sec.3.6: yaw damper. | Not implemented - decided against (2026-09-21). | **SKIPPED** (by decision) |
