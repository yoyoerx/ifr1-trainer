...
| R10 | Sec.4.1 p.4-3: turn-rate limit **90% of standard rate** (HDG, NAV, APR, REV, CWS; piston), **GPSS 90/110/130%** by hardware mod code. | F50: `Commands.turn_rate_dps` -> `SimModel.turn_rate_limit`: HDG 90%, NAV/APR/REV 90/45/15% by stage. **GPSS still 100%** (POH 90/110/130% by hardware code - pick one). | **PARTIAL** |
