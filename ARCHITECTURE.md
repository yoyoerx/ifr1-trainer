...
| `datasrc/faa.py` | Resolve per-cycle FAA URLs (CIFP candidate names; NASR by scraping the landing page; `artcc`/`airspace` are date-stamped directly). Fetch CIFP (`FAACIFP18`) + NASR CSVs into `data/faa/<ident>/`, sha256, write `manifest.json`, zip-slip guard. Two opt-in kinds (never in the default `cifp,nasr`, F62): `artcc` (`AFF.txt` -> `artcc.json` via `datasrc/aff.py`) and `airspace` (Class Airspace shapefile -> `airspace.json` via `datasrc/shp.py`, ~150 MB download). CLI: `status` / `update [--cycle] [--kinds] [--force]` / `list`. HTTP behind `_http_get*` for test stubbing. | `airac`, stdlib `urllib`/`zipfile` | **done (19 tests, live-verified)** |
| `datasrc/aff.py` | Stdlib regex parser for `AFF.txt` (ARTCC RCAG ground-station sites + VHF frequencies) - anchor-based, not column-sliced (no documented field widths). | stdlib `re` | **done (F62)** |
| `datasrc/shp.py` | Stdlib ESRI Shapefile (.shp/.dbf) reader (Polygon/PolygonZ only) + an iterative Douglas-Peucker simplifier, so the FAA's Class Airspace shapefile doesn't need pyshp/shapely. `extract_class_airspace` keeps Class B/C/D, drops Class E. | stdlib `struct` | **done (F62)** |
| `navdata/` | Load a nav database into memory. `model.py` (loader-agnostic types: `Waypoint`/`VhfNavaid`/`NdbNavaid`/`Runway`/`Airport`/`LegType`/`ProcedureLeg`/`Procedure`/`AirwayPoint`/`Airway`/`CenterSite`/`Fss`/`Airspace`/`NavDatabase` with `find`/`nearest_*`), `arinc424.py` (ARINC 424-18 field parsers), `cifp.py` (parse `FAACIFP18` — sections D VOR/DME, DB NDB, EA/PC waypoints, PA airports, PG runways, **PI airport localizers** with course + runway, PD/PE/PF procedures, ER airways), `nasr.py` (merge `FRQ.csv` comm frequencies + `APT_RWY.csv` surface/lighting + `FRQ.csv` RCO rows onto airports/`db.fss` + `FRQ.csv` "CLASS B"/"CLASS C" rows and each airport's own tower onto `db.airspace_controlling`, F64), `__init__.load()` (also reads the opt-in `artcc.json`/`airspace.json` into `db.centers`/`db.airspaces`, F62). **Sole source: FAA CIFP + NASR** — no X-Plane fallback (`navdata/xplane.py` removed 2026-09-11, see §5.2); `load()` raises `FileNotFoundError` with the fix-it command if no cycle is cached. `load()` **pickle-caches the parsed DB** to `data/faa/<cycle>/navdb-<hash>.pkl` (keyed on CIFP size+mtime + `comms`/`areas` + schema version, currently 7) → ~2 s cold, ~0.7 s warm; procedure legs stay **lazy** and are not pickled. | `navmath`, `datasrc` | **DONE — CIFP + NASR comm merge + nearest-N + DB cache. Follow-ups (transitions split, VOR-only filter) parked in WORKING.md.** |
| `faa2xp/` | Offline CLI: back-port the cached FAA CIFP into an X-Plane 12 install's `Custom Data/` (`extract.py` CIFP -> X-Plane rows, `merge.py` format + merge over default `earth_*.dat`, `install.py` reversible install/restore with manifest). Never imported by the runtime loop. See §5.4. |
| `gpsnav.py` | **The trainer's core** — the variant-independent GPS/RNAV navigator. `FlightPlan` (append/insert/delete/activate-leg), `DirectTo` (frozen course; resumes into the plan if the target is on it; `cancel_direct_to` re-picks the closest leg), auto waypoint sequencing w/ turn anticipation (capped — `tan(x/2)` blows up near a 180° reversal) → SUSP at the end **and at the MAP / manual legs**; **a hold is actually flown, not just suspended** (`_start_hold`/`_step_hold`/`_complete_hold_lap`: the AIM 5-3-9 entry via `navmath.hold_entry`, then synthetic outbound/inbound legs through the normal dtk/xtk `NavState` fields so `follow_leg`/a coupled AP fly it unmodified; a single-circuit `HF` auto-continues once re-established inbound, `HM`/`HA` repeat until a hold-aware `toggle_suspend()` arms a release at the next inbound crossing) (`PlanWaypoint.stop_here`, latched), OBS (`toggle_obs` / `nudge_obs`), CDI source GPS/VLOC, **phase-of-flight CDI full-scale** (`NavState.cdi_scale_nm`, gradual 5.0→1.0→0.30 nm slew), `NEXT DTK`/`TURN TO` staging, nav-data-expiry annunciation + message queue. `load_procedure` assembles a CIFP procedure and **synthesises a fix for every non-fix ARINC leg** (`_expand_leg`: CA/VA/FA/CD/VD/FD/FC/CR/VR/CI/VI/PI/HM/HA/FM/VM + DME/RF arcs, which are *not* synthesised: the fix ending an arc carries `arc_centre`/`arc_turn` and `update()` flies the circle - tangent DTK, off-circle XTK, arc-length DTG, F45), so a full published approach loads; it also resolves an `RW..` MAP fix to the runway threshold (`_RwyFix` / `_resolve_proc_fix`) and stages the approach frequency (`approach_freq` / `approach_ref` via `_approach_vloc_freq` — runway `ils_ident`, else the first post-FAF `recnav_ident`) for `main` to drop into VLOC standby. `update(pos, track, gs, dt=None)` → `NavState`. `PageCursor` NAV·WPT·AUX·NRST + big/small-knob (clamped) + CRSR; `handle_event` is a **page-aware dispatch** — the Select Direct-To dialog (`DirectToEntry`) is a modal overlay: ENT always confirms the typed identifier first (`DirectToEntry.confirming` → "Activate?" highlighted, knobs locked out) and a **second** ENT actually activates (`_confirm`) — per the Pilot's Guide sec.4.1, this is true even re-centring on the already-active waypoint, no single-ENT shortcut; CLR while confirming backs out to editing rather than closing the page. The Flight Plan page edits/adds/deletes waypoints in place; WPT pages type an identifier (`wpt_entry` + `lookup()`); NRST pages scroll `nearest_for_page()` — WPT/NRST ENT flies a Direct-To directly (a documented page-specific shortcut, separate from the DCT-key dialog above). Each `PlanWaypoint` carries `proc_kind`/`proc_ident` (`""` for a plain enroute fix) marking which loaded procedure, if any, it belongs to — `load_procedure` tags every leg it adds and, loading a second procedure of a kind already present, first calls `remove_procedure(kind)` (Pilot's Guide p.56-57: Select Approach?/Arrival?/Departure? "replace the current ... with a new selection" — legs don't stack). CLR on the Flight Plan page no longer deletes on the spot: it opens a `_remove_confirm` modal (`REMOVE WAYPOINT`, p.49; or, on a row belonging to a loaded procedure, `REMOVE <procedure>` — CLR on any of that procedure's legs removes the whole thing, standing in for the title-row shortcut on p.59) — ENT on "Yes?" applies it, CLR cancels with nothing changed (F67). The **PROC key** opens a second modal overlay, `ProcSelect` (`begin_proc_select` / `_proc_event`): a MENU → PROC → TRANS → **LOADACT** wizard (Pilot's Guide sec.5 p.61 step 5: "highlight 'Load?' or 'Activate?' (approaches only) and press ENT" - `_proc_begin_loadact`) that ends in `load_procedure` (VECTORS = final segment only) and, for "Activate?", an immediate `_activate_approach` in the same pass rather than a separate PROC-menu round trip; **Activate Approach / Vectors-To-Final** remain available from the MENU step too, for re-activating something already loaded (F34) — reactivating one **already being flown**, before its MAP, instead opens a `_restart_confirm` modal (`RESTART APPROACH`, p.62) rather than silently resetting progress; past the MAP, or before first activation, it proceeds unconfirmed exactly as before (F67). The **MNU key** opens a third modal overlay, `FplMenu`, on the Flight Plan / Flight Plan Catalog pages: **Invert Flight Plan** (`invert_flight_plan` — reverses waypoint order, reactivates leg 1), **Copy Flight Plan** (stores the active plan, or a selected catalog entry, into the first empty catalog slot), **Sort Catalog** (`catalog_sort` — alphabetizes by comment, empties packed last), **Delete Flight Plan** (clears the active plan, or a catalog slot), and — only for a kind of procedure currently loaded — **Remove Approach/Arrival/Departure** (p.58, opens the same `_remove_confirm` modal as the CLR shortcut, via `remove_procedure`). The **Flight Plan Catalog** page (`fpl_catalog`, 19 stored plans = FPL 01-19; FPL 00 is always the active `self.fpl`) lists each slot's `FlightPlan.comment` (auto "ORIGIN/DEST", or `catalog_set_comment`) + waypoint count; cursor-on ENT recalls a slot as the active plan and jumps to Flight Plan, CLR deletes it in place (`catalog_store`/`catalog_store_first_empty`/`catalog_load`/`catalog_delete`). `crossfill(other)` copies the active plan to another `GpsNav` instance (no live 2nd FMS unit in `main.World` yet). The **VNAV page** (`VnavProfile`/`VnavStatus`, ALT bezel key → `PageCursor.go_to_vnav`) programs a target fix/altitude/descent-angle picked from the remaining flight plan (cursor-on outer = field, inner = value); `vnav_status(alt_ft)` reports distance-to-target, top-of-descent distance and vertical deviation once armed — deliberately decoupled from `update()`/`NavState` (altitude isn't otherwise part of this core's state) so no existing caller's signature changed. The **Charts page** (`chart_airport_sel`/`chart_sel`) tracks browse position only — no I/O in this pure core; ENT there is intercepted in `main.route_event` (`_open_selected_chart`) before `handle_event` even sees it, since opening a PDF is real file I/O, not avionics state. `Variant` + `VARIANT_530` / `VARIANT_430`. | `navmath`, `navdata`, `ifr1` | **done — core + 115 tests (108 gns530 scenario + 7 gpsnav)** |
| `gns530.py` / `gns430.py` | Thin views over `gpsnav.GpsNav`: `class Gns530(GpsNav)` binds `VARIANT_530`, `class Gns430(GpsNav)` binds `VARIANT_430`; `class Gns530W(GpsNav)` binds `VARIANT_530W` (the WAAS 530W: `Variant.waas` is a capability flag - LPV/L-VNAV `glidepath()`, level-of-service annunciations, 2 nm enroute / angular approach CDI scaling - not a fork of the state machine; path-point data from `navdata.cifp`); each re-exports the shared types so `from gns530 import …` keeps working. **No avionics logic** — the state machine does not fork; only the display (screen height, visible-row count, faceplate art) differs. | `gpsnav` | **done (M8)** |
...
| `androidbridge/` | Android-port scaffolding (see `docs/ANDROID_PORT_PLAN.md`) — the Kotlin-facing seam onto the brain, called via Chaquopy from `android/app/.../bridge/BrainBridge.kt`. `TrainerSession` wraps one `GpsNav` + one `SimModel`, `tick(dt)` returns a **plain dict** (no dataclass/Point instances) for cheap JNI marshalling; `selftest.py::run` is the exact entry point Kotlin calls to prove the whole `navmath`→`navdata`→`gpsnav`→`sim_model` chain imports and runs on-device. Pure Python, no Chaquopy/Android import of its own — exercised by the normal desktop suite like everything else. Does **not** yet replicate `main.py`'s `_fms_bezel` key/knob dispatch table (tracked as follow-up, not silently dropped). | `gpsnav`, `navdata`, `sim_model`, `navmath` | **Phase 0 scaffold (4 tests)** |
| `android/` | Chaquopy-hybrid Android app shell (Kotlin/Compose), **unbuilt/unverified** — no Android SDK/JDK 17/Gradle in this dev environment; see `android/README.md`. `stageBrainPython` (`app/build.gradle.kts`) copies the curated brain-module list from the repo root at build time (single source of truth, no hand-duplicated Python). `BrainBridge.kt` starts Chaquopy and calls `androidbridge.selftest`; `DrawCommand.kt`/`InstrumentCanvas.kt` are the §3.6 draw-command-list contract shape + a Kotlin-side Canvas replay (no real GNS layout data produced yet); `UsbHidInput.kt` is an empty stub pending the §3.1 hardware spike. | Android SDK, Chaquopy, Compose | **Phase 0 scaffold, not yet opened/synced** |
| `tests/` | pytest. `navmath`, `airac`, `navdata` parsers get real coverage; `gpsnav` / `instruments` / `sim_model` get scenario + sign-convention tests; `wmm` is checked against NOAA's published table; `gdl90` CRC against the spec example. `faa.py`/`wx.py` network calls are mocked; CIFP tests use verbatim public-domain record lines as fixtures. | `pytest` | 860 passing (wmm 103, navmath 67, airac 67, gns530 108, ifr1 47, cifp 36, render/main 101, arinc424 35, nasr 27, instruments 28, radios 25, wx 33, sim_model 21, autopilot 19, faa 19, windsaloft 14, wx_auto 13, config 20, foreflight_discovery 13, dtpp 21, navdata_query 11, gdl90 9, xplane_feed 8, gpsnav 7, scoring 6) |
...
- **Middle** — NAV1 and NAV2 as Bendix/King-style round CDI heads
  ...
  same snapshot `draw_six_pack` itself reads) plus BARO (`Scene.baro_inhg`
  - "stack" has no altimeter dial of its own the way "steam"'s six-pack
  does, so this is the only place it's shown at all) are in
